...
class ViS(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(self, x):
        ...
        B, N, D = x.shape  # N is the number of patches

        # Resize positional embeddings to match N
        if self.pos_emb1D.size(0) != N:
            pos_emb = nn.functional.interpolate(
                self.pos_emb1D.unsqueeze(0).permute(0, 2, 1),  # [1, D, L]
                size=N,
                mode='linear',
                align_corners=False
                ).permute(0, 2, 1).squeeze(0)  # [N, D]
        else:
            pos_emb = self.pos_emb1D

        x = x + pos_emb.unsqueeze(0)  # [1, N, D] broadcasted across batch
