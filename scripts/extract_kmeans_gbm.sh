#!/bin/bash
#SBATCH --job-name=kmeanft_gbm
#SBATCH --output=/projects/conco/gundla/root/uniglacier/logs/sequoia/extract_kmeans_gbm_%A_%a.out
#SBATCH --error=/projects/conco/gundla/root/uniglacier/logs/sequoia/extract_kmeans_gbm_%A_%a.err
...
# Process TCGA-GBM
echo "Processing TCGA-GBM dataset"
TOTAL_SLIDES=$(grep "TCGA-GBM" ./examples/ref_file_missing.csv | wc -l)
SPLITS=5
...
echo "Processing TCGA-GBM slides from index $START_INDEX to $END_INDEX"
echo "Feature output directory: $FEATURE_PATH/TCGA-GBM"
...
# Check exit status
if [ $? -ne 0 ]; then
    echo "Error: Python script failed for GBM"
    exit 1
fi