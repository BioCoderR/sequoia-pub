#!/bin/bash
#SBATCH --job-name=ViSlgg
#SBATCH --output=/projects/conco/gundla/root/uniglacier/logs/sequoia/vis/visualize_lgg_GOI_out_%j.out
#SBATCH --error=/projects/conco/gundla/root/uniglacier/logs/sequoia/vis/visualize_lgg_GOI_err_%j.err
...
# FOLDS="0,1,2,3,4"
# Read only LGG rows from CSV
mapfile -t SLIDE_IDS < <(awk -F, '$4 == "TCGA-LGG" {print $1}' $CSV_FILE | tail -n +2)
mapfile -t PROJECTS < <(awk -F, '$4 == "TCGA-LGG" {print $4}' $CSV_FILE | tail -n +2)
...
CHECKPOINT_DIR="/projects/conco/gundla/root/uniglacier/models/trained/image2st/predictions/sequoia_lgg"
